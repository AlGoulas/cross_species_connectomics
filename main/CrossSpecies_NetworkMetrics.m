...
%
%The script performs a bulk of tasks (see option what_to_do) for analyzing
%brain conenctomes and extracting cross-species, cross-levels principles.
%
%Input:
%
...
%           area. NOTE Currently only the macaque monkey (Species(4) and 
%           Species(7) have actual empirical data for the DV axis)
%
...
%                       'statistical_energy': 2D statistical energy test
%
%
%Output:
%
...
%For 'core-periphery':
%       
%       max_clique_size: the size of largest cliques in the connectome. The
%                        union of these cliques is the core.
%       core_indexes: Nx1 vector with 1s denoting the core areas and 0s the
%                     periphery areas.
%
%       max_clique_size_pval: p-value that corresponds to a comaprison of
%                             the size of the largest clique in the 
%                             original connectome and the size of the 
%                             largest clique in null connectomes (edge,
%                             node, degree matched).
%
%       stat_cyto_core_periphery: statistical test value for deciphering if
%                                 the core and periphery areas differ
%                                 in terms of their cytology (ks stat or 
%                                 statistical energy based on the 
%                                 'compare_distributions' parameter)
%       
%       pval_cyto_core_periphery: p-value corresponding to the 
%                                 aforementioned comparison  
%
%       All the above (stats and pval) hold as well for comparisons of the 
%       core-periphery with respect to:
%
%       geometric centrality(geo), anterior-posterior (ap),
%       dorsal-ventral(dv), lateral-medial (lm) axes, wiring cost and 
%       projection length 
%
%       pca_coeff:  The coefficients from the PCA applied to the network
%                   topology profiles.
%       pca_score:  The scores from the PCA applied to the network
%                   topology profiles.
%       pca_explained:  a vector denoting the percentage of variance
%                       explained ny each PC (starting with PC1).
%       net_metrics: The network metrics computed for each area. 
%       net_metrics_names: The names of the network metrics used.
%
%       rho_pc_cyto_partial: Partial Spearman's rank correlation for 
%                            the relation between a PC (PC1 default) and 
%                            the cytology of areas, by taking into account
%                            the anterior-posterior coordinate of areas. 
%       pval_pc_cyto_partial:The corresponding p-value of the above test.
%       rho_pc_cyto: Spearman's rank correlation (bivariate) for examining 
%                    the relation between a PC (PC1 default) and 
%                    the cytology of areas.
%       pval_pc_cyto:The corresponding p-value of the above test.
%       rho_pc_ap_partial:  As for the cyto, but for the anterior-posterior
%                           axis.
%       pval_pc_ap_partial: As for the cyto, but for the anterior-posterior
%                           axis.
%       rho_pc_ap: As for the cyto, but for the anterior-posterior axis.
%       pval_pc_ap: As for the cyto, but for the anterior-posterior axis.
%
%
%Plotting:
%
%           For all what_to_do options, it contains all the data that were 
%           plotted in the figures, for saving them or remaking figures.
%--------------------------------------------------------------------------
...
for i=indexes
    ...
    if(any(strcmp('connectome-cyto-geo-ap', what_to_do)))
        ...
        Connectome_CrossLevels_Stats(meter).pval_pc_cyto_partial=p;
        ...
        Connectome_CrossLevels_Stats(meter).pval_pc_ap_partial=p;
