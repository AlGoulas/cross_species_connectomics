...
function [p, phi_nm, phi_nm_boot] = minentest(x,y,nboot)
...
%Avoid having the logarithm of 0 values (It happens when differences are exactly 0)
%So replace the zeros with a tiny number (1.0e-10).
...
distxy=pdist2(x,y,'euclidean');

distx(distx==0)=1.0e-10;
a=-log(distx);

disty(disty==0)=1.0e-10;
b=-log(disty);

distxy(distxy==0)=1.0e-10;
c=-log(distxy);
